...
Модули, которые предназначены для использования в других модулях, обычно не содержат код между ``start`` и ``end``. Подробнее об этом будет рассказано при более подробном знакомстве с модулями.
